...
public class Controller implements Serializable {
    ...
    public List<Horario> getHorariosMostrados() {
        ...
        return listaHorarios.stream().filter(h -> diaFiltro.equals(h.getDia())).collect(Collectors.toList());
    }
    ...
    public void cargarCursos() { listaCursos = new ArrayList<>(); try (Connection c = getConnection(); Statement s = c.createStatement(); ResultSet r = s.executeQuery("SELECT * FROM cursos")) { while (r.next()) listaCursos.add(new Curso(r.getString("codigo"), r.getString("nombre"), r.getInt("creditos"), r.getString("docente_asignado"))); } catch (Exception e) { manejarError(e); } }
    ...
    public void cargarAulas() { listaAulas = new ArrayList<>(); try (Connection c = getConnection(); Statement s = c.createStatement(); ResultSet r = s.executeQuery("SELECT * FROM aulas")) { while (r.next()) listaAulas.add(new Aula(r.getString("numero"), r.getString("tipo"), r.getInt("capacidad"))); } catch (Exception e) { manejarError(e); } }
